...
## Date: September 2026
## Description: Currently in Movebank the same track can be found in multiple studies. The duplications of tracks can be exact, but often other studies contain only part of the tracking period, or be thinned to a coarser fix frequency, or both. In this scrip these duplicated tracks are identified from the timestamps and coordinates.  Only files of the same species are compared with each other; files not associated to a species are excluded. The track with either the longest duration or the highest number of gps locations is retained. 
## INPUT : The folder you obtained in Step 3 of the script ̀`1_Download_data_from_Movebank.R, i.e. one folder containing one .rds file per individual
## OUTPUT: 
## - `reference_table_all_studies.rds`: one row per individual, containing a set of metadata information, this is the inventory of files before duplicated tracks are identified. Individuals without taxon information will be flagged here, the column `species` will be NA and these individuals will be ignored in the duplication detection. Advice: add the taxon name to the Movebank study or ask the owner of the study to do so. 
## - `duplicated_tracks.csv`: same table as above plus the result of the duplicate detection. Used to identify which individuals to keep.   keep == FALSE are the files to exclude
## - `duplicated_tracks_pairs.csv`: one row per pair of files that was compared location by location, use the grouping to check borderline cases.
## - `duplicated_tracks.rds`: the complete result object of find_duplicated_tracks() that will be used for plot_duplicate_groups(). Can be used to get the duplicated plots without needing to rerun all the code
## - `duplicatePlots`: contains one plot per duplicated group, these are organized in one folder per species
...
##
# Rationale of the location comparison (checked on real Movebank data):
...
#  - Tags of *different* animals from the same study often share the exact same
#    fix schedule (e.g. every 5 min at hh:m0:50), so timestamps alone are not
#    enough: the coordinates of those time-coincident fixes are km apart.
...
#
# Output of find_duplicated_tracks(): list with
#   $tracks  reference table, one row per file (columns of extract_track_info()),
#            plus the columns
...
#____________________________________________________________________________
## Usage ####
# This script is meant to be sourced. Copy the lines below into the script
# where it is executed and adjust the settings at the top. All folders are
# created by these lines; only the folder with the rds files must exist.
#
# path_to_rds_folder <- "path_to_folder_with_rds_files/"   # one move2 object (one individual) per rds file
# path_to_results    <- "path_to_folder_for_results/"      # tables are written here
...
# res <- find_duplicated_tracks(files = path_to_rds_folder, n_cores = n_cores, cache_dir = path_to_cache,
...
# plot_duplicate_groups(result = res, out_dir = path_to_plots)                          # one jpg per duplicate group
...
#
# Output files written by the lines above:
#   reference_table_all_studies.rds  One row per rds file, written right after step 1, before any
#                                    duplicate detection: fileName, path, MBid, individual_local_identifier,
...
#                                    timestamp_ms, xmin/xmax/ymin/ymax, read_error, no_species. The
#                                    inventory of all files, independent of the duplicate analysis; on
#                                    disk even if a later step fails.
...
#                                    kept file first) and $settings (the thresholds used). Input of
#                                    plot_duplicate_groups(); reload it to continue without re-running.

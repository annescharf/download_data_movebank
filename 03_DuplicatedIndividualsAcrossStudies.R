...
referenceTableStudies <-  function(path_to_indv_move2){ 
  ...
  indTable <- as.data.frame(rbindlist(lapply(tagList, function(tag){
    ...
    rdf <- data.frame(
      ...
      species = if(!is.null(tag_td$taxon_canonical_name)){as.character(tag_td$taxon_canonical_name)}else{NA},
      # species = if(!is.null(tag_td$taxon_canonical_name)){tag_td$taxon_canonical_name}else{tag_td$taxon_ids}, # this is a quick fix, but problem arises when study contains multiple species, therefore decided to assign NA (line), and check manually the study
      ...
      median_timelag_mins = median(mt_time_lags(tag, "min"),na.rm=T),
      min_timelag_mins = min(mt_time_lags(tag, "min"),na.rm=T)
    )
    
    units(rdf$tracking_duration_days) <- units::make_units(day)
    rdf$median_timelag_mins <- round(rdf$median_timelag_mins)
    rdf$min_timelag_mins <- round(rdf$min_timelag_mins,2)
    return(rdf)
  })))
  return(indTable)
}
...
fls <- list.files(pthClean, pattern="rds", full.names = T)
...
sdf <- data.frame(sort(table(referenceTableStudies_ALL$species)))
table(is.na(referenceTableStudies_ALL$species))

referenceTableStudies_ALL[is.na(referenceTableStudies_ALL$species),c("fileName","MBid")] ## go to movebank and check

# referenceTableStudies_ALL$species[referenceTableStudies_ALL$fileName=="1659758323_GriffonSardinia01.rds"] <- "Gyps fulvus"


## ensure only keeping individuals of wanted species:
vultSps <- read.csv("/home/ascharf/Documents/Projects/Drylands/vultureSps.csv")
...
sort(unique(referenceTableStudies_ALL$animal_life_stage))
ageNA <- referenceTableStudies_ALL[!is.na(referenceTableStudies_ALL$animal_life_stage),]
length(unique(ageNA$MBid))
...
library(dplyr)
library(bit64)
referenceTableStudies_ALL <-  readRDS(paste0(pathTOfolder,"/referenceTableStudies_ALL_original.rds"))
...
pathTOfolder <- "path_to_folder_where_all_data_will_be_saved/"
pthClean <- paste0(pathTOfolder,"2.vultureIndv_mv2_clean_empty_duply/")
...
pdf(file=paste0(pathTOfolder,"duplicatePlots.pdf"), width=20, height=12)
